...
# fourth chart shows rural population vs arable land
    ...
    graph_five = []
    df_bar = cleandata('data/API_SP.RUR.TOTL_DS2_en_csv_v2_9914824.csv')
    df_bar.columns = ['country','year','rural_population']
    df_bar.sort_values('rural_population', ascending=False, inplace=True)
    df_bar = df_bar[df_bar['year'] == 2015] 

    graph_five.append(
      go.Bar(
      x = df_bar.country.tolist(),
      y = df_bar.rural_population.tolist(),
      )
    )
    ...
    keep = ["Country Name"]+[str(x)for x in range(1995, 2020)]
    valuevars = [str(x)for x in range(1995, 2020)]
    
    df2 = cleandata("data/API_SL.AGR.EMPL.FE.ZS_DS2_en_csv_v2_1433067.csv",keep,valuevars)
    
    countrylist = df2.country.unique().tolist()
    
    for country in countrylist:
        x_val = df2[df2["country"]==country].year.tolist()
        y_val = df2[df2["country"]==country].variable.tolist()
        year = df2[df2['country'] == country].year.tolist()
        country_label = df2[df2['country'] == country].country.tolist()
        ...
        #append multiple traces to list
        graph_six.append(
            go.Scatter(
              x = x_val,
              y = y_val,
    ...
    layout_six = dict(title = 'Percentage of female employment <br> in Agriculture 1995-2019',
            xaxis = dict(title = 'Year'),
